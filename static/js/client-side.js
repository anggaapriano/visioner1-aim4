$(document).ready(function(){
  ...
  function display_tombol_pilihan_peta(pilihanPeta) {
    ...
    // Update Event Listener untuk Tombol Pilihan Peta 6		
    $("#tombol_peta6").click(function(e) {
      e.preventDefault();
			...
			$.ajax({
			  ...
			  success:function(res){
				...
				if(!robot){
					...
					$('#tombol_peta6').prop("disabled", false);
					
					// Tampilkan Status Permainan agar pemain/pengguna
					// dapat menggerakkan agent (Si Jagung)
					pesan = 'Silahkan gerakkan Si Jagung'
  ...
  function cek_menang(state,robot) {
	...
	if(state == 15) {		
		// Non-aktifkan tombol kendali agent (Si Jagung)
		...
		if(robot) {
			pesan = 'CornnyBot Menang 😎'
			display_status_permainan(pesan);
		} else {
			pesan = 'Selamat Anda Menang 🤩'
			display_status_permainan(pesan);
		}
	}
  }	  
  ...
  // Fungsi memanggil API untuk menggerakkan Agent (Si Jagung)
  ...
  $("#tombol_gerak_kanan").click(function(e) {
    ...
    // Panggil fungsi untuk menggerakkan Agent (Si Jagung)
    kirim_gerak(gerak);    
  })
  ...
  // Fungsi ketika tombol peta 6 ditekan
  $("#tombol_peta6").click(function(e) {
    e.preventDefault();
